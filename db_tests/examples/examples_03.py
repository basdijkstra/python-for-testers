...
def create_database():
    ...
    cursor = conn.cursor()
    cursor.execute("CREATE TABLE customer (id INTEGER NOT NULL, first_name TEXT NOT NULL, last_name TEXT NOT NULL)")
    cursor.execute("INSERT INTO customer VALUES (1, 'John', 'Smith')")
    cursor.executemany("INSERT INTO customer VALUES (?,?,?)", users)
    cursor.execute("CREATE TABLE cust_order (orderid INTEGER PRIMARY KEY NOT NULL, description TEXT NOT NULL, customer INTEGER NOT NULL, FOREIGN KEY(customer) REFERENCES customer(id))")
